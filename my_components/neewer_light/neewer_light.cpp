...
void NeewerLight::write_state(light::LightState *state) {
  ...
  if (light_state) {
    ...
    float color_temperature = state->remote_values.get_color_temperature();

    // Convert from mired to Kelvin
    uint16_t kelvin = (uint16_t) roundf(1000000.0f / color_temperature);
    uint8_t color_temperature_code = (uint8_t) (kelvin / 100);
    if (kelvin % 100 > 50) {
      color_temperature_code += 1;
    }

    ESP_LOGD(TAG, "C: 0x%02x G: 0x%02x Brightness: 0x%02x CT Dec: %d", this->channel_, this->group_,
             converted_brightness, color_temperature_code);
    ...
    std::vector<uint8_t> color_set_command = {0x7E, 0xA5, this->group_, converted_brightness,
...
void NeewerLight::send_command(const std::vector<uint8_t> &data) {
  ...
  checksum_data = data;
  checksum_data.push_back((uint8_t) (checksum & 0xFF));
